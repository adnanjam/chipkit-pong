...
void move_pad(int direction, int* offset, int colOffset1, int colOffset2){
    ...
    *offset += direction;
    ...
    if(*offset > 31 - height) *offset = 31 - height;
}

void move_players(void){
  if(getsw() == 1){
    if(getbtns() == 1)move_pad(-1, &padAOffset, 0, 1);
    if(getbtns() == 2)move_pad(1, &padAOffset, 0, 1);
  }else {
    if(getbtns() == 1)move_pad(-1, &padBOffset, -1, -2);
    if(getbtns() == 2)move_pad(1, &padBOffset, -1, -2);
  }
}
...
void move_ballX(void){
  if(right == 1){
    ballx++;
    if(ballx >= 127) reset_ball();
  }

  if(right == 0){
    ballx--;
    if(ballx <= 1) reset_ball();
  }
}
